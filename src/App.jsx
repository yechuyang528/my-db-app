 ...
 * 数据库系统  ·  完整保真导入版
 *
 * 依赖：npm install xlsx     (用于 Excel/ODS 文件导入和导出)
 *
 * 本版本针对“xlsx 导入不完整 / 数据失真”问题重写了导入内核：
 *   1. 新增「完整保真导入」模式（默认开启）
 *        - 按工作表真实范围(!ref)逐格读取，一行不漏、一列不裁
 *        - 取 Excel 的“显示文本”原样导入，不做任何类型转换
 *          → 前导零、长 ID、科学计数、超长小数、原始日期格式全部保留
 *   2. 合并单元格自动展开填充，不再出现空洞
 *   3. 空行可选择保留（保真模式默认保留）
 *   4. 表头三种模式：自动识别 / 指定某行 / 无表头(列1…列N)
 *   5. 支持“一次导入全部工作表”，每个工作表生成一个数据库
 *   6. 预览区显示工作表、总行数、列数、是否有行被过滤的提示
 * ============================================================ */
...
function detectFieldType(samples) {
  ...
  // 注意：带前导零或超 15 位的数字串不当作数字，避免精度/格式丢失
...
/* -------- 值转换 --------
   fidelity=true（保真）：原样转字符串，绝不改写数值/日期，零丢失
   fidelity=false（智能）：按目标类型做温和转换 */
function coerceValue(raw, type, fidelity = false) {
  // Date 对象 → YYYY-MM-DD（仅智能模式才会进入；保真模式 raw 已是显示文本）
...
function buildFullGrid(XLSX, sheet, fidelity) {
  ...
  // 1) 先按真实范围铺出完整网格（行数=范围全部行，列数=范围全部列）
  ...
  for (let R = range.s.r; R <= range.e.r; R++) {
    for (let C = range.s.c; C <= range.e.c; C++) {
      ...
      let val;
      if (fidelity) {
        // 保真：优先用 Excel 显示文本(.w)，与你在 Excel 里看到的完全一致
        ...
        // 智能：保留原始值（含 Date 对象），交给 detect/coerce 处理
  ...
  // 2) 展开合并单元格：把左上角的值填进整个合并区域，消除“空洞”
...
/* sheet → { fields, records, ... }
   options:
     fidelity     完整保真（默认 true）：不转换类型、不丢任何值
     headerMode   "auto" | "manual" | "none"
     headerRow    headerMode=manual 时的表头行(0-based)
     keepBlankRows 是否保留空行（保真默认 true）
*/
...
async function parseUploadedFile(file, options = {}) {
  ...
  if (isExcel) {
    ...
    // cellNF/cellText 让 SheetJS 计算出 .w 显示文本，保真模式需要它
    ...
    return {
      ...
      realFormat,
      _wbReadOpts: { codepage: readOpts.codepage } // 供“全部工作表”复用
    };
  }
...
export default function App() {
  ...
  const [expandedGroups, setExpandedGroups] = useState(new Set()); // 展开的文件夹
  ...
  /* 导入选项（保真相关） */
  const [fidelityMode, setFidelityMode] = useState(true);   // 完整保真，默认开启
  const [headerMode, setHeaderMode] = useState("auto");     // auto | manual | none
  ...
  /* 一次导入全部工作表：每个工作表 → 一个数据库 */
  /* 把整本 xlsx 导入为【一个文件夹】：
     - 每个工作表仍是独立的表（结构、数据互不干扰）
     - 所有表归到同一个 group（文件夹），侧边栏可折叠展开
     - 文件夹名默认取文件名，可在“数据库名称”里改 */
  ...
  const deleteGroup = async (group) => {
    ...
    if (!window.confirm(`确认删除文件夹「${group}」？其中 ${names.length} 张表及全部数据将被清除。`)) return;
  ...
  return (
    <div style={{ display: "flex", height: "100%", minHeight: 560, fontFamily: "system-ui, sans-serif", fontSize: 14, background: "#fff", color: "#1a1a1a" }}>
      {/* Sidebar */}
      <div style={{ width: 220, borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", flexShrink: 0, background: "#fafafa" }}>
          ...
          {(() => {
            ...
            const renderItem = (name, inFolder) => (
              <div key={name} onClick={() => openDb(name)}
                ...
                <span style={{ color: activeDb === name ? "#1d4ed8" : "#6b7280", fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: inFolder ? 120 : 130 }}>
                  {inFolder ? "📄 " : ""}{metas[name].display || name}
                </span>
                <span style={{ fontSize: 11, color: "#9ca3af", background: "#f3f4f6", padding: "1px 6px", borderRadius: 10, border: "1px solid #e5e7eb" }}>
                  ...
                  return (
                    <div key={"grp-" + g}>
                      <div
                        ...
                        style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px 7px 14px", cursor: "pointer", background: "#f3f4f6", borderTop: "1px solid #e5e7eb", borderBottom: "1px solid #e5e7eb" }}>
                        ...
                        <span style={{ fontSize: 10, color: "#9ca3af" }}>{groups[g].length}表</span>
                        <button onClick={(e) => { e.stopPropagation(); deleteGroup(g); }}
                          title="删除整个文件夹"
                          style={{ border: "none", background: "transparent", color: "#d1d5db", cursor: "pointer", fontSize: 12, padding: "0 2px" }}>🗑</button>
                      </div>
